...
def export_xls(request):
    ...
    if request.method=='GET':
        ...
        if id == '1':
            response['Content-Disposition'] = 'attachment;filename=01 Deployed.xls'  # 指定返回文件名
            ...
            sql = sql_base +  """ and ( hosttype_id ='%s' or hosttype_id='%s' or hosttype_id= '%s' ) 
             and (host_status = '%s' or host_status='%s' ) and (p.host_name not like '%s' and p.host_name not like '%s'  ) and p.site_id='%s' order by h.id, p.host_name """ % (hosttype_id1, hosttype_id2, hosttype_id3, host_status1, host_status2,IBM_ASS_PC,IBM_ASS_LT,site_id )
        ...
        #02 Servers （服务器清单）
        if id == '2':
            response['Content-Disposition'] = 'attachment;filename=02 Servers.xls'  # 指定返回文件名
            # sql = """select p.host_name ,p.service_tag,h.name , p.host_model,p.receive_date , CURDATE(),TIMESTAMPDIFF(YEAR,p.receive_date,CURDATE()) , \
            #         TIMESTAMPDIFF(MONTH,p.receive_date,CURDATE())%%12 ,u.full_name ,b.bl_name, d.dept_name, s.sitename as location,p.host_status,  p.asset_code,p.remark\
            #         from deviceman_pc_list as p, deviceman_user_list as u, deviceman_dept_list as d, deviceman_hosttype as h, deviceman_site as s , deviceman_bl_list as b \
            #         where p.user_list_id=u.id and u.dept_list_id=d.id and p.site_id=s.id and p.hosttype_id=h.id and b.id=d.bl_list_id and ( hosttype_id ='%s' ) \
            #         and (host_status = '%s' or host_status='%s' ) and p.site_id='%s' order by h.id, p.host_name """ % (hosttype_id4, host_status1, host_status2,site_id)
            #
            sql = sql_base + """and ( hosttype_id ='%s' ) and (host_status = '%s' or host_status='%s' ) and p.site_id='%s' order by h.id, p.host_name """ % (hosttype_id4, host_status1, host_status2,site_id)

            sheet = wb.add_sheet(u'02 Servers')  # excel里添加类别

        #03 IBM Assets （IBM资产清单）
        if id == '3':
            response['Content-Disposition'] = 'attachment;filename=03 IBM Assets.xls'  # 指定返回文件名
            # sql = """select p.host_name ,p.service_tag,h.name , p.host_model,p.receive_date , CURDATE(),TIMESTAMPDIFF(YEAR,p.receive_date,CURDATE()) , \
            #         TIMESTAMPDIFF(MONTH,p.receive_date,CURDATE())%%12 ,u.full_name , b.bl_name,d.dept_name, s.sitename as location,p.host_status,  p.asset_code,p.remark \
            #         from deviceman_pc_list as p, deviceman_user_list as u, deviceman_dept_list as d, deviceman_hosttype as h, deviceman_site as s, deviceman_bl_list as b \
            #         where p.user_list_id=u.id and u.dept_list_id=d.id and p.site_id=s.id and p.hosttype_id=h.id and b.id=d.bl_list_id  \
            #         and host_name like '%s' and p.site_id='%s' order by h.id, p.host_name """ % (IBM_ASS,site_id)
            #
            sql = sql_base + """and (host_name like '%s' or  host_name like '%s' )and p.site_id='%s' order by h.id, p.host_name """ % (IBM_ASS_PC, IBM_ASS_LT,site_id)
